...
class AdminService {
  ...
  async createAdmin({ name, phone, email, plan, society, createdBy }) {
    ...
    const signupLink = `${process.env.FRONTEND_URL || 'http://localhost:5173'}/admin-signup?phone=${phone}&key=${licenseKey}`;
...
Please sign up using this link: ${signupLink}`);
  ...
  async createSuperSubAdmin({ name, phone, email, permissions = [] }, requestingUser) {
    if (!name || !phone || !email) {
      throw new ApiError(400, "Name, phone, and email are required");
    }
    ...
    const existingUser = await User.findOne({
      $or: [{ phone }, { email }],
      role: { $in: ["super_sub_admin", "SUPER_SUB_ADMIN"] },
    });
    if (existingUser) {
      throw new ApiError(409, "Super Sub Admin with this phone or email already exists");
    }
    ...
    const emailHtml = `
      <h2>Welcome to IntefAI Security, ${name}!</h2>
      <p>You have been invited as a Super Sub Admin.</p>
      <p><strong>Your License Key is:</strong> ${licenseKey}</p>
      <p>Please complete your sign up by setting your password using the link below:</p>
      <a href="${signupLink}">Complete Sign Up</a>
    `;
    ...
    const emailSent = await sendEmail({
      to: email,
      subject: "IntefAI Security - Super Sub Admin License Key & Invitation",
      html: emailHtml,
    });
  ...
  async createSubAdmin(
    { name, phone, email, password, permissions = [], assignedAdminId },
    requestingUser,
  ) {
    ...
    if (existingByEmail) {
      throw new ApiError(409, "Sub-Admin with this email already exists");
    }
    ...
    const emailHtml = `
      <h2>Welcome to IntefAI Security, ${name}!</h2>
      <p>You have been created as a Sub Admin.</p>
      <p><strong>Login ID (Email):</strong> ${email}</p>
      <p><strong>Password:</strong> ${password}</p>
      <p>Please log in using the link below:</p>
      <a href="${loginLink}">Login to Dashboard</a>
    `;

    const emailSent = await sendEmail({
      to: email,
      subject: "IntefAI Security - Sub Admin Credentials",
      html: emailHtml,
    });

    const result = (await User.findById(subAdmin._id)
      .select("-password")
      .populate("society", "name address")).toObject();
  ...
  async createSecretary(
    { name, phone, email, password, society, assignedAdminId },
    requestingUser,
  ) {
    if (!name || !phone || !email || !password || !society) {
      throw new ApiError(
        400,
        "Name, phone, email, password, and society are required",
      );
    }
    ...
    const emailHtml = `
      <h2>Welcome to IntefAI Security, ${name}!</h2>
      <p>You have been created as a Society Secretary.</p>
      <p><strong>Login ID (Email):</strong> ${email}</p>
      <p><strong>Password:</strong> ${password}</p>
      <p>Please log in using the link below:</p>
      <a href="${loginLink}">Login to Dashboard</a>
    ...
    const emailSent = await sendEmail({
      to: email,
      subject: "IntefAI Security - Society Secretary Credentials",
      html: emailHtml,
    });

    const result = (await User.findById(secretary._id)
      .select("-password")
      .populate("society", "name address")).toObject();
