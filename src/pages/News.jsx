...
const News = () => {
  ...
  const filteredNews = useMemo(() => {
    ...
    return newsList.filter((n) => {
      ...
      const soc = typeof n.society === 'object' ? n.society?.name || '' : n.society || '';
      return t.toLowerCase().includes(term) || c.toLowerCase().includes(term) || cat.toLowerCase().includes(term) || soc.toLowerCase().includes(term);
    });
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setFormError(getErrorMessage(err, editingItem ? 'Failed to update news.' : 'Failed to publish news item.'));
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="page-container">
      <div className="page-header">
        <div>
          ...
          <p className="page-description">Publish announcements, maintenance updates, and society notices</p>
        </div>
        <div className="action-buttons-group">
          <button className="btn btn-secondary" onClick={fetchNewsList} disabled={isLoading} title="Refresh data">
      ...
      {isLoading ? (
        <div className="card-box flex-center p-8"><LoadingSpinner text="Loading community news..." /></div>
      ) : paginatedNews.length === 0 ? (
        <div className="card-box empty-state-box"><h3>No News Published</h3></div>
      ) : (
        <>
                ...
                {paginatedNews.map((item) => {
                  ...
                  return (
                    <tr key={id} className="table-row-hover">
                      <td><span className="font-semibold">{itemTitle}</span></td>
                      <td><span className="badge-category">{itemCat}</span></td>
                      {isSuperAdmin && <td><span className="badge-category">{societyName}</span></td>}
                      <td>{item.createdAt ? new Date(item.createdAt).toLocaleDateString() : 'Recent'}</td>
        ...
        <div className="modal-backdrop" onClick={() => setIsModalOpen(false)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '520px' }}>
            <div className="modal-header">
              <h3>{editingItem ? 'Edit News Article' : 'Add Community News'}</h3>
            ...
            <form onSubmit={handleSave}>
              <div className="modal-body">
                ...
                <div className="form-group mb-4">
                  ...
                  >
                    <option value="General Society Notice">General Society Notice</option>
                    <option value="Scheduled Water Supply Maintenance">Scheduled Water Supply Maintenance</option>
                    <option value="Elevator Maintenance & Servicing">Elevator Maintenance & Servicing</option>
                    <option value="Power Outage & Generator Servicing">Power Outage & Generator Servicing</option>
                    <option value="Pest Control Drive Schedule">Pest Control Drive Schedule</option>
                    <option value="Society Maintenance Fee Due Notice">Society Maintenance Fee Due Notice</option>
                    <option value="Annual General Body Meeting (AGM)">Annual General Body Meeting (AGM)</option>
                    <option value="Festival Celebration Announcement">Festival Celebration Announcement</option>
                    <option value="Security & Visitor Protocol Update">Security & Visitor Protocol Update</option>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary" disabled={isSaving}>{isSaving ? 'Publishing...' : editingItem ? 'Update News' : 'Publish'}</button>
              </div>
            </form>
          </div>
        </div>
