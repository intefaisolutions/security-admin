...
const SocietySecretaries = () => {
  const { user } = useAuth();
  const isSuperAdmin = user?.role === "super_admin";
  const canManageAdmins = ["super_admin", "super_sub_admin", "admin", "sub_admin"].includes(user?.role);
  const canManageThisType = ["super_admin", "super_sub_admin", "admin", "sub_admin"].includes(user?.role);
  const userSocietyId =
  ...
  const fetchAdminsList = async () => {
    ...
    try {
      ...
      setAdmins(adminList.filter(a => a.role === "secretary"));
    } catch (err) {
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <div>
          <h1 className="page-title">Society Secretaries Management</h1>
          <p className="page-description">
            Create, update, and manage Society Secretary accounts
          </p>
        </div>
...
export default SocietySecretaries;
