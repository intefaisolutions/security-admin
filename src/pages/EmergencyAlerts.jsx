...
import { getAlerts, getAlertById, createAlert, deleteAlert } from '../api/admin';
...
const EmergencyAlerts = () => {
  ...
  const filteredAlerts = useMemo(() => {
    ...
    return alerts.filter((item) => {
      ...
      const s = typeof item.society === 'object' ? item.society?.name || '' : item.society || '';
      return t.toLowerCase().includes(term) || d.toLowerCase().includes(term) || s.toLowerCase().includes(term);
    });
    ...
    <div className="page-container">
      <div className="page-header">
        <div>
          ...
          <p className="page-description">Broadcast and monitor urgent security & medical alerts</p>
        </div>
        <div className="action-buttons-group">
          <button className="btn btn-secondary" onClick={fetchAlertsList} disabled={isLoading} title="Refresh data">
      ...
      {isLoading ? (
        <div className="card-box flex-center p-8"><LoadingSpinner text="Loading emergency alerts..." /></div>
      ) : paginatedAlerts.length === 0 ? (
        <div className="card-box empty-state-box"><h3>No Emergency Alerts</h3></div>
      ) : (
        <>
                ...
                {paginatedAlerts.map((item) => {
                  ...
                  return (
                    <tr key={id} className="table-row-hover">
                      ...
                      <td><span className="badge-unit" style={{ background: 'rgba(239, 68, 68, 0.2)', color: '#f87171' }}>{severityText}</span></td>
                      {isSuperAdmin && (
                        <td><span className="badge-category">{societyName}</span></td>
                      )}
                      <td><StatusBadge status={status} /></td>
                      <td>{item.createdAt ? new Date(item.createdAt).toLocaleString() : 'Recent'}</td>
                      <td className="text-right">
                        <div className="action-buttons-group">
                          <button className="btn btn-sm btn-outline" onClick={() => handleOpenDetail(id, item)}>View</button>
        ...
        <div className="modal-backdrop" onClick={() => setIsCreateOpen(false)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '520px' }}>
            <div className="modal-header">
              ...
              <button className="icon-btn-close" onClick={() => setIsCreateOpen(false)}>&times;</button>
            </div>
            <form onSubmit={handleCreate}>
              <div className="modal-body">
                {createError && <div className="alert alert-danger mb-4"><span>{createError}</span></div>}
                ...
                <div className="form-group mb-4">
                  ...
                  >
                    <option value="Fire Emergency Alert">Fire Emergency Alert</option>
                    <option value="Medical Emergency SOS">Medical Emergency SOS</option>
                    <option value="Security Breach & Intruder Alert">Security Breach & Intruder Alert</option>
                    <option value="Elevator / Lift Trapped Emergency">Elevator / Lift Trapped Emergency</option>
                    <option value="Gas Leakage Emergency">Gas Leakage Emergency</option>
                    <option value="Water Pipeline Burst Alert">Water Pipeline Burst Alert</option>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary" disabled={isSaving}>{isSaving ? 'Broadcasting...' : 'Send Alert'}</button>
              </div>
            </form>
          </div>
        </div>
